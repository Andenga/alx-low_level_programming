...
/**
 * print_number - Prints a number (integer) with the - flag character.
...
/**
 * print_string - Prints a string with the - flag character.
...
int print_string(va_list args)
{
    ...
    /* Print trailing spaces if necessary */
    int field_width = 10;  // Example field width value
    int str_len = strlen(str);
    if (str_len < field_width) {
        int num_spaces = field_width - str_len;
        for (int i = 0; i < num_spaces; i++) {
            _putchar(' ');
            printed_chars++;
        }
